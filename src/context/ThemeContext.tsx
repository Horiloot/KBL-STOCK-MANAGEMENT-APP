...
export const ThemeProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [isDark, setIsDark] = useState<boolean>(() => {
    const savedMode = (localStorage.getItem('potato_app_mode') as ThemeMode) || 'light';
  ...
  useEffect(() => {
    ...
    const applyTheme = () => {
      const activeIsDark = resolveIsDark();
      ...
      // Apply color scale variables directly to root element
      const palette = THEME_PALETTES[theme] || THEME_PALETTES['corporate-light'];
      Object.entries(palette.scale).forEach(([step, val]) => {
        root.style.setProperty(`--color-sky-${step}`, val);
      });
  ...
  const setTheme = (newTheme: ThemeName) => {
    ...
    if (newTheme === 'corporate-dark') {
      setModeState('dark');
      localStorage.setItem('potato_app_mode', 'dark');
    }
  };
